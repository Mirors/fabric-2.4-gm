...
func (ks *fileBasedKeyStore) GetKey(ski []byte) (k bccsp.Key, err error) {
	...
	case "sk":
		...
		switch key.(type) {
		case *sm2.PrivateKey:
			return &sm2.Sm2PrivateKey{key.(*sm2.PrivateKey)}, nil
		default:
	...
	case "pk":
		...
		switch key.(type) {
		case *sm2.PublicKey:
			return &sm2.Sm2PublicKey{key.(*sm2.PublicKey)}, nil
		default:
...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) {
	...
	for _, f := range files {
		...
		//将key强转为从
		PrivKey := &sm2.PrivateKey{sm2.PublicKey(key.PublicKey), key.D}
...
func (ks *fileBasedKeyStore) createKeyStoreIfNotExists() error {
	...
	if missing {
		logger.Debugf("KeyStore path [%s] missing [%t]: [%s]", ksPath, missing, ErrToString(err))
