...
func (s *Serializer) IsMetadataSerialized(namespace, name string, structure interface{}, state OpaqueState) (bool, error) {
	...
	return bytes.Equal(util.ComputeSM3(metadataBin), existingMDHash), nil
}
...
func (s *Serializer) IsSerialized(namespace, name string, structure interface{}, state OpaqueState) (bool, error) {
	...
	if !bytes.Equal(util.ComputeSM3(metadataBin), existingKeys[metadataKeyName]) {
		return false, nil
	}
	...
	for i := 0; i < value.NumField(); i++ {
		...
		if existingValue, ok := existingKeys[keyName]; !ok || !bytes.Equal(existingValue, util.ComputeSM3(marshaledFieldValue)) {
			return false, nil
		}
	}
